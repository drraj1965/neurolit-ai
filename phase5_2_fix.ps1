...
Write-Host "=== FIXING SEARCH (API + DATE FILTER) ==="

$BackupDir = "_phase5_2_fix_backup"
...
class ApiService {
  ...
  Future<List<Article>> searchArticles(String query, String dateFilter) async {
    ...
    for (var node in nodes) {
      ...
      final abstract = node.findAllElements("AbstractText")
          .map((e) => e.text)
          .join(" ");
