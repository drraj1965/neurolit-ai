...
class LaymanService {
  ...
  String buildPrompt(String mode, List<Map<String, dynamic>> payload) {
    ...
    if (mode == "quick") {
      return """
You are preparing a clear layperson-friendly explanation based on MULTIPLE research articles.
...
- short, clear lines
- suitable for a non-specialist reader
...
""";
    ...
    if (mode == "detailed") {
      return """
You are preparing a DETAILED LAYMAN EXPLANATION based on MULTIPLE research articles.
...
""";
  ...
  Future<Map<String, dynamic>> generate({
    ...
    final literatureMode = articles.isNotEmpty
        ? articles.first.literatureMode
    ...
    final folder = await workspaceService.getSummariesDirectory(
      mode: literatureMode,
      date: now,
      topic: topic,
    );

    final file = File("${folder.path}/layman_${mode}_$hash.txt");
