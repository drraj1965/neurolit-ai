import 'dart:io';

import '../models/article.dart';
import '../models/literature_mode.dart';
import 'mode_workspace_service.dart';

class SavedCollection {
  final File file;
  final String name;
  final DateTime? created;
  final int declaredCount;
  final int parsedCount;
  final LiteratureMode mode;

  SavedCollection({
    required this.file,
    required this.name,
    required this.created,
    required this.declaredCount,
    required this.parsedCount,
    required this.mode,
  });

  String get fileName => file.uri.pathSegments.last;
}

class CollectionService {
  final ModeWorkspaceService workspaceService;

  CollectionService({
    ModeWorkspaceService? workspaceService,
  }) : workspaceService = workspaceService ?? ModeWorkspaceService();

  Future<Directory> getCollectionDirectory(LiteratureMode mode) {
    return workspaceService.getCollectionsDirectory(mode);
  }

  Future<List<SavedCollection>> listCollections(LiteratureMode mode) async {
    final modeDir = await workspaceService.getCollectionsDirectory(
      mode,
      create: false,
    );
    final legacyDir = await workspaceService.getLegacyCollectionsDirectory(
      create: false,
    );
    final directories = <Directory>[modeDir];

    if (legacyDir.path != modeDir.path) {
      directories.add(legacyDir);
    }

    final files = <File>[];
    for (final dir in directories) {
      if (!await dir.exists()) {
        continue;
      }

      final dirFiles = await dir
          .list()
          .where(
            (entity) =>
                entity is File && entity.path.toLowerCase().endsWith('.txt'),
          )
          .cast<File>()
          .toList();
      files.addAll(dirFiles);
    }

    files.sort((a, b) => b.lastModifiedSync().compareTo(a.lastModifiedSync()));

    final collections = <SavedCollection>[];
    for (final file in files) {
      final collection = await inspectCollection(file);
      if (collection.mode == mode) {
        collections.add(collection);
      }
    }

    return collections;
  }

  Future<SavedCollection> inspectCollection(File file) async {
    final text = await file.readAsString();
    final articles = parseCollectionText(text);
    final collectionMode = _inferCollectionMode(text, articles);

    final nameMatch = RegExp(
      r'^Name:\s*(.+)$',
      multiLine: true,
    ).firstMatch(text);
    final createdMatch = RegExp(
      r'^Created:\s*(.+)$',
      multiLine: true,
    ).firstMatch(text);
    final countMatch = RegExp(
      r'^Count:\s*(\d+)$',
      multiLine: true,
    ).firstMatch(text);

    final fallbackName = _nameFromFile(file);

    return SavedCollection(
      file: file,
      name: (nameMatch?.group(1) ?? fallbackName).trim(),
      created: DateTime.tryParse((createdMatch?.group(1) ?? '').trim()),
      declaredCount:
          int.tryParse((countMatch?.group(1) ?? '').trim()) ?? articles.length,
      parsedCount: articles.length,
      mode: collectionMode,
    );
  }

  Future<List<Article>> loadArticles(File file) async {
    final text = await file.readAsString();
    return parseCollectionText(text);
  }

  List<Article> parseCollectionText(String text) {
    final blocks = text.split(RegExp(r'\r?\n-{10,}\r?\n'));
    final articles = <Article>[];

    for (final rawBlock in blocks) {
      final lines = rawBlock.split(RegExp(r'\r?\n'));
      final firstTitleIndex = lines.indexWhere(
        (line) => line.startsWith('TITLE:'),
      );
      if (firstTitleIndex == -1) continue;

      final articleLines = lines.sublist(firstTitleIndex);
      final metadata = <String, String>{};
      final abstractLines = <String>[];
      var readingAbstract = false;

      for (final line in articleLines) {
        if (!readingAbstract && line.trim().isEmpty) {
          readingAbstract = true;
          continue;
        }

        if (!readingAbstract) {
          final separatorIndex = line.indexOf(':');
          if (separatorIndex == -1) continue;

          final key = line.substring(0, separatorIndex).trim().toUpperCase();
          final value = line.substring(separatorIndex + 1).trim();
          metadata[key] = value;
          continue;
        }

        abstractLines.add(line);
      }

      final title = (metadata['TITLE'] ?? '').trim();
      final internalId = (metadata['ID'] ?? metadata['PMID'] ?? '').trim();
      final source = (metadata['SOURCE'] ?? '').trim();
      final mode = literatureModeFromValue(
        (metadata['MODE'] ??
                (source.isNotEmpty && source.toLowerCase() != 'pubmed'
                    ? 'engineering'
                    : 'medical'))
            .trim()
            .toLowerCase(),
      );
      final identifierLabel = (metadata['IDENTIFIER_LABEL'] ??
              (metadata.containsKey('PMID') ? 'PMID' : 'Identifier'))
          .trim();
      final identifierValue = (metadata['IDENTIFIER_VALUE'] ??
              metadata['PMID'] ??
              internalId)
          .trim();
      final abstractText = abstractLines.join('\n').trim();
      final pdfUrl = (metadata['PDF_URL'] ?? '').trim();
      final pmcId = (metadata['PMC_ID'] ?? metadata['PMC'] ?? '').trim();
      final hasFullText = (metadata['HAS_FULL_TEXT'] ?? '').trim().toLowerCase() ==
              'true' ||
          pdfUrl.isNotEmpty ||
          pmcId.isNotEmpty;

      if (title.isEmpty && internalId.isEmpty && abstractText.isEmpty) continue;

      final link = (metadata['LINK'] ?? '').trim().isNotEmpty
          ? metadata['LINK']!.trim()
          : (mode == LiteratureMode.medical && identifierValue.isNotEmpty
              ? 'https://pubmed.ncbi.nlm.nih.gov/$identifierValue/'
              : '');

      articles.add(
        Article(
          title: title.isNotEmpty ? title : 'Untitled article',
          abstractText: abstractText,
          journal: (metadata['JOURNAL'] ?? '').trim(),
          date: (metadata['DATE'] ?? '').trim(),
          authors: (metadata['AUTHORS'] ?? '').trim(),
          pmid: internalId.isNotEmpty
              ? internalId
              : 'collection-${articles.length + 1}',
          link: link,
          source: source,
          pdfUrl: pdfUrl,
          hasFullText: hasFullText,
          canDownloadFullText: hasFullText,
          literatureMode: mode,
          identifierLabel: identifierLabel.isEmpty ? 'Identifier' : identifierLabel,
          identifierValue: identifierValue,
          pmcId: pmcId,
          isPMC: pmcId.isNotEmpty,
          isFree: hasFullText,
        ),
      );
    }

    return articles;
  }

  Future<File> saveCollection({
    required LiteratureMode mode,
    required String name,
    required Iterable<Article> articles,
  }) async {
    final dir = await getCollectionDirectory(mode);
    final now = DateTime.now();
    final fileName =
        "${_safeFileName(name)}_${now.toIso8601String().replaceAll(":", "-")}.txt";
    final file = File("${dir.path}/$fileName");
    final buffer = StringBuffer();

    buffer.writeln("=== COLLECTION ===");
    buffer.writeln("Name: $name");
    buffer.writeln("Created: $now");
    buffer.writeln("Mode: ${mode.storageValue}");
    buffer.writeln("Count: ${articles.length}");
    buffer.writeln("==================\n");

    for (final a in articles) {
      buffer.writeln("TITLE: ${a.title}");
      buffer.writeln("AUTHORS: ${a.authors}");
      if (a.journal.trim().isNotEmpty) buffer.writeln("JOURNAL: ${a.journal}");
      if (a.date.trim().isNotEmpty) buffer.writeln("DATE: ${a.date}");
      buffer.writeln("MODE: ${a.literatureMode.storageValue}");
      buffer.writeln("SOURCE: ${a.displaySource}");
      buffer.writeln("ID: ${a.selectionKey}");
      buffer.writeln("IDENTIFIER_LABEL: ${a.displayIdentifierLabel}");
      buffer.writeln("IDENTIFIER_VALUE: ${a.displayIdentifierValue}");
      if (a.literatureMode == LiteratureMode.medical) {
        buffer.writeln("PMID: ${a.displayIdentifierValue}");
      }
      if (a.link.trim().isNotEmpty) buffer.writeln("LINK: ${a.link}");
      if (a.pdfUrl.trim().isNotEmpty) buffer.writeln("PDF_URL: ${a.pdfUrl}");
      if (a.pmcId.trim().isNotEmpty) buffer.writeln("PMC_ID: ${a.pmcId}");
      buffer.writeln("HAS_FULL_TEXT: ${a.hasFullText || a.canDownloadFullText}");
      buffer.writeln();
      buffer.writeln(a.abstractText);
      buffer.writeln("\n------------------------------------\n");
    }

    await file.writeAsString(buffer.toString());
    return file;
  }

  Future<File> renameCollection(File file, String newName) async {
    final text = await file.readAsString();
    final updatedText = text.contains(RegExp(r'^Name:', multiLine: true))
        ? text.replaceFirst(
            RegExp(r'^Name:\s*.*$', multiLine: true),
            'Name: $newName',
          )
        : text.replaceFirst(
            '=== COLLECTION ===',
            '=== COLLECTION ===\nName: $newName',
          );

    await file.writeAsString(updatedText);

    final newPath = await _uniquePath(
      file.parent,
      '${_safeFileName(newName)}.txt',
    );
    return file.rename(newPath);
  }

  Future<void> deleteCollection(File file) async {
    if (await file.exists()) {
      await file.delete();
    }
  }

  String _nameFromFile(File file) {
    final baseName = file.uri.pathSegments.last.replaceFirst(
      RegExp(r'\.txt$'),
      '',
    );
    return baseName.replaceAll('_', ' ');
  }

  String _safeFileName(String input) {
    final cleaned = input
        .replaceAll(RegExp(r'[<>:"/\\|?*]'), '')
        .replaceAll(RegExp(r'\s+'), '_')
        .trim();
    return cleaned.isEmpty ? 'Collection' : cleaned;
  }

  Future<String> _uniquePath(Directory dir, String fileName) async {
    final dotIndex = fileName.lastIndexOf('.');
    final base = dotIndex == -1 ? fileName : fileName.substring(0, dotIndex);
    final extension = dotIndex == -1 ? '' : fileName.substring(dotIndex);
    var candidate = File("${dir.path}/$fileName");

    if (!await candidate.exists()) return candidate.path;

    var counter = 2;
    while (true) {
      candidate = File("${dir.path}/${base}_$counter$extension");
      if (!await candidate.exists()) return candidate.path;
      counter++;
    }
  }

  LiteratureMode _inferCollectionMode(String text, List<Article> articles) {
    final modeMatch = RegExp(
      r'^Mode:\s*(.+)$',
      multiLine: true,
      caseSensitive: false,
    ).firstMatch(text);
    final modeValue = (modeMatch?.group(1) ?? '').trim();
    if (modeValue.isNotEmpty) {
      return literatureModeFromValue(modeValue);
    }

    var engineeringCount = 0;
    for (final article in articles) {
      if (article.literatureMode == LiteratureMode.engineering) {
        engineeringCount++;
      }
    }

    return engineeringCount > (articles.length / 2)
        ? LiteratureMode.engineering
        : LiteratureMode.medical;
  }
}
